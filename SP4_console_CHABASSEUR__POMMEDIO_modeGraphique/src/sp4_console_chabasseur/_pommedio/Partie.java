        ...
        while(FinDePartie==false){
            ...
            if (repJoueur==1){
                ...
                if (NumLigneJouer == -1){
                    ...
                    if (plateau.presenceTrouNoir(NumLigneJouer, NumColJouer)==true && plateau.presenceDesintegrateur(NumLigneJouer, NumColJouer)==true){
                ...
                if (plateau.presenceJeton(NumLigneJouer, NumColJouer)== true ){   //
                    if (plateau.lireCouleurJeton(NumLigneJouer,NumColJouer)==joueurCourant.getCouleur()){
                        plateau.supprimerJeton(NumLigneJouer, NumColJouer);           //
                        joueurCourant.ajouterJetons(new Jeton(joueurCourant.getCouleur()));    
                        plateau.tasserColonne(NumColJouer); 
                    }else{
                        System.out.println("Vous ne pouvez retirez que les jetons de votre couleur!");
                    }
                }
